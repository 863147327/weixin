    ...
    <div v-if="goods_detail">
        <swiper indicator-dots autoplay circular indicator-active-color="#fff">
            <block v-for="item in goods_detail.pics" :key="item.pics_id">
                <swiper-item>
                    <image @click="preview(item.pics_big)" mode="aspectFill" :src="item.pics_big"></image>
                </swiper-item>
            </block>
        </swiper>
        <!-- 2.0 商品的基本信息 -->
        ...
        <view class="tabs">
            <view class="tabs-head">
                <view @click="tabSelect(index)" v-for="(item,index) in tabs"
                      :class="[currentTabIndex=== index ? 'active':'','tabs-item']" :key="index">
                    <text>{{item}}</text>
                </view>
            </view>
            <view class="tabs-body">
                ...
                <view v-show="currentTabIndex === 1">
                    <view v-for="(item,index) in goods_detail.attrs" :key="item.attr_id" class="param-item">
                        <text :class="['note',index===goods_detail.attrs.length-1?'param-item-last':'']">
                            {{item.attr_name}}
                        </text>
                        <text :class="['description',index===goods_detail.attrs.length-1?'param-item-last':'']">
                            {{item.attr_vals}}
                        </text>
                    </view>
                </view>
  ...
  export default {
    ...
    data () {
      return {
        goods_detail: null,
        address: null, // 收货地址
        tabs: ['图文介绍', '规格参数'], // tabs栏数据
        currentTabIndex: 0 // 当前选中tab下标
      }
    },
    onLoad (options) {
      console.log(options)
